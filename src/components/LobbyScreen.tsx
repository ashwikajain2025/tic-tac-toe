...
export function LobbyScreen({ onReady, onLeaderboard }: LobbyScreenProps) {
  ...
  // Communities list for the join step
  const [allCommunities, setAllCommunities]     = useState<Community[]>([]);
  // My communities for the dashboard step
  const [memberships, setMemberships]           = useState<{ community: Community; player: PlayerProfile }[]>([]);

  const resetErrors = () => setError('');

  // ── Fetch communities list whenever join step is shown ─────────────────────
  useEffect(() => {
    if (step !== 'join') return;
    setLoading(true);
    getAllCommunities()
  ...
  // ── Fetch my communities whenever my-communities step is shown ─────────────
  useEffect(() => {
    if (step !== 'my-communities') return;
    setLoading(true);
    getCommunitiesForPlayer(myName)
      .then(setMemberships)
      .catch((e) => setError(e.message))
      .finally(() => setLoading(false));
  }, [step, myName]);

  // ── Real-time: watch for 2nd player joining in waiting step ────────────────
  const waitingCommunityRef = useRef<Community | null>(null);
  waitingCommunityRef.current = waitingCommunity;

  useEffect(() => {
    if (step !== 'waiting' || !waitingCommunity) return;

    const channel = supabase
      .channel(`community-players-${waitingCommunity.id}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
          schema: 'public',
          table: 'players',
          filter: `community_id=eq.${waitingCommunity.id}`,
        },
        async () => {
          // A new player just joined — check if we now have 2
          const pair = await getCommunityPlayers(waitingCommunity.id).catch(() => null);
    ...
    return (
      <div className="flex flex-col items-center gap-6 w-full">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">Community multiplayer</p>
        </div>

        <div className="flex flex-col gap-4 w-full">
          <Field label="Your name" value={myName}
            onChange={(v) => { setMyName(v); resetErrors(); }}
            placeholder="e.g. Alice" onEnter={handleContinue} />
  ...
  if (step === 'my-communities') {
    const handlePlay = async (community: Community) => {
      resetErrors();
      try {
        const pair = await getCommunityPlayers(community.id);
        if (pair) {
          onReady(community, pair[0], pair[1]);
        } else {
          const me = memberships.find((m) => m.community.id === community.id);
          if (me) {
            setWaitingCommunity(community);
            setWaitingPlayer(me.player);
            setSecondName('');
            setStep('waiting');
          }
        }
      } catch (e: unknown) {
        setError(e instanceof Error ? e.message : 'Could not enter community.');
      }
    };
    ...
    return (
      <div className="flex flex-col gap-5 w-full">
        <div>
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500 mt-0.5">Your communities</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-2">
            {memberships.map(({ community }) => {
              const pair   = null; // optimistic — resolved on click
              const isFull = (community.playerIds as string[]).length >= 2;
              void pair;
              return (
                <div key={community.id}
                  ...
                  <div className="min-w-0">
                    <p className="font-bold text-slate-800 dark:text-slate-100 truncate">{community.name}</p>
                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                      {isFull ? 'Both players joined' : 'Waiting for second player…'}
                    </p>
                  </div>
                  <button onClick={() => handlePlay(community)}
                    className={[
        ...
        <div className="flex flex-col gap-2">
          <button onClick={() => { resetErrors(); setCommunityName(''); setStep('create'); }}
            className="w-full py-3 rounded-xl font-semibold text-sm bg-violet-600 hover:bg-violet-700 text-white active:scale-95 transition-all duration-150 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-violet-400">
            + Create a Community
          </button>
          <button onClick={() => { resetErrors(); setCommunityName(''); setStep('join'); }}
        ...
        <button onClick={() => { resetErrors(); setMyName(''); setStep('home'); }}
  ...
  if (step === 'create') {
    const handleCreate = async () => {
      ...
      setLoading(true);
      try {
        const existing = await findCommunityByName(name);
        if (existing) { setError('A community with that name already exists. Try joining it instead.'); return; }
        const community = await createCommunity(name);
        const player    = await joinCommunity(community.id, myName.trim());
        setWaitingCommunity(community);
    ...
    return (
      <FormCard title="Create a Community" subtitle="Choose a name for your group"
        onBack={() => { resetErrors(); setStep('my-communities'); }}
        onSubmit={handleCreate} submitLabel="Create" error={error} loading={loading}>
        <Field label="Community name" value={communityName} onChange={setCommunityName}
          placeholder="e.g. Weekend Warriors" onEnter={handleCreate} />
  ...
  if (step === 'join') {
    ...
    return (
      <div className="flex flex-col gap-5 w-full">
        <div>
          <button onClick={() => { resetErrors(); setCommunityName(''); setStep('my-communities'); }}
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500 mt-0.5">Pick one from the list or type its name</p>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          {loading ? <Spinner /> : allCommunities.length === 0 ? (
            <div className="text-center py-8 text-slate-400 dark:text-slate-500 text-sm border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl">
            ...
            <div className="flex flex-col gap-2 max-h-52 overflow-y-auto pr-0.5">
              {visible.map((c) => {
                ...
                return (
                  <button key={c.id} disabled={isFull}
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className={['font-semibold text-sm truncate',
                          isSelected ? 'text-rose-600 dark:text-rose-400' : 'text-slate-700 dark:text-slate-200'].join(' ')}>
                          {c.name}
                        </p>
        ...
        <button onClick={handleJoin} disabled={loading}
          className="w-full py-3 rounded-xl font-semibold text-sm bg-rose-500 hover:bg-rose-600 disabled:opacity-60 text-white active:scale-95 transition-all duration-150 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-rose-400">
  ...
  if (step === 'waiting' && waitingCommunity && waitingPlayer) {
    const handleSecondPlayer = async () => {
      resetErrors();
      ...
      if (pName.toLowerCase() === waitingPlayer.name.toLowerCase()) {
        setError('Both players must have different names.'); return;
      }
      setLoading(true);
      try {
        await joinCommunity(waitingCommunity.id, pName);
        const pair = await getCommunityPlayers(waitingCommunity.id);
        if (pair) onReady(waitingCommunity, pair[0], pair[1]);
      } catch (e: unknown) {
        setError(e instanceof Error ? e.message : 'Could not join.');
      } finally {
    ...
    return (
      <div className="flex flex-col items-center gap-6 w-full">
        <div className="text-center">
          <h2 className="text-2xl font-extrabold text-slate-800 dark:text-white">Waiting for friend…</h2>
          <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">
            Share the community name{' '}
            <span className="font-bold text-violet-600 dark:text-violet-400">"{waitingCommunity.name}"</span>
            {' '}or add them here directly
          </p>
        </div>
        ...
        <div className="w-full bg-violet-50 dark:bg-violet-900/20 border-2 border-violet-200 dark:border-violet-700 rounded-xl p-4 text-center">
          ...
          <p className="text-lg font-bold text-violet-700 dark:text-violet-300">{waitingPlayer.name}</p>
        </div>

        <div className="w-full flex flex-col gap-3">
          <Field label="Second player's name (O)" value={secondName}
            onChange={(v) => { setSecondName(v); resetErrors(); }}
            placeholder="e.g. Bob" onEnter={handleSecondPlayer} />
          {error && <p className="text-xs text-rose-500 font-semibold">{error}</p>}
          <button onClick={handleSecondPlayer} disabled={loading}
            className="w-full py-3 rounded-xl font-semibold text-sm bg-violet-600 hover:bg-violet-700 disabled:opacity-60 text-white active:scale-95 transition-all duration-150 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-violet-400">
            {loading ? 'Starting…' : 'Start Game'}
          </button>
          <button onClick={() => { resetErrors(); setStep('my-communities'); }}
            className="text-xs text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 text-center transition-colors">
...
interface FormCardProps {
  ...
  submitLabel: string; error: string; loading: boolean;
  children: React.ReactNode;
}

function FormCard({ title, subtitle, onBack, onSubmit, submitLabel, error, loading, children }: FormCardProps) {
  return (
    <div className="flex flex-col gap-5 w-full">
      <div>
